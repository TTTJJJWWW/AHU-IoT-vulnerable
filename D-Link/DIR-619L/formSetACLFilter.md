...
In the D-Link DIR-619L B1 2.06 firmware has a buffer overflow vulnerability in the `formSetACLFilter` function. The `v17` variable receives the `curTime` parameter from a POST request. However, since the user can control the input of `curTime`, the `sprintf` can cause a buffer overflow vulnerability.

![image-20240927134446570](https://raw.githubusercontent.com/abcdefg-png/images2/main/image-20240927134446570.png)
